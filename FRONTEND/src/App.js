...
import {
  ...
  Copy, Bookmark, AlertCircle, Clock, Square, Play
} from 'lucide-react';
...
const TypewriterText = ({ text, speed = 50, onComplete, onTextUpdate, shouldStop, onStop, shouldResume, startFromIndex = 0 }) => {
  const [displayedText, setDisplayedText] = useState(text.slice(0, startFromIndex));
  const [currentIndex, setCurrentIndex] = useState(startFromIndex);
  const [isStopped, setIsStopped] = useState(false);

  useEffect(() => {
    if (shouldResume && isStopped) {
      setIsStopped(false);
      return;
    }
    ...
    if (shouldStop && !isStopped) {
      setIsStopped(true);
      if (onStop) {
        onStop(displayedText, currentIndex);
      }
      return;
    }
    ...
    } else if ((currentIndex >= text.length || isStopped) && onComplete && !shouldStop) {
      onComplete();
    }
  }, [currentIndex, text, speed, onComplete, onTextUpdate, shouldStop, shouldResume, isStopped, displayedText, onStop, startFromIndex]);
...
const ChatbotUI = () => {
  ...
  const [pausedMessage, setPausedMessage] = useState(null); // Store paused message data
  ...
  const handleStopGeneration = () => {
    ...
    } else if (pausedMessage) {
      // Resume generation from where it was stopped
      resumeGeneration();
    }
  };
  ...
  const resumeGeneration = () => {
    if (pausedMessage) {
      ...
      // Update the message to resume animation
      updateChatMessages(activeChat, prev => 
        prev.map(msg => 
          msg.id === pausedMessage.messageId 
            ? { ...msg, isAnimating: true, text: pausedMessage.fullText }
            : msg
        )
      );
      
      // Reset resume flag after a brief moment
      setTimeout(() => {
  ...
  const handleTypewriterStop = (currentText, currentIndex) => {
    // Find the message being animated and get its full text
    const animatingMessage = currentMessages.find(msg => msg.isAnimating);
    if (animatingMessage) {
      // Store paused message data
      setPausedMessage({
        messageId: animatingMessage.id,
        fullText: animatingMessage.text,
        currentText: currentText,
        pausedAtIndex: currentIndex
      });

      // Update the message with the current text and mark it as not animating
      updateChatMessages(activeChat, prev => 
        prev.map(msg => 
          msg.id === animatingMessage.id 
            ? { ...msg, text: currentText, isAnimating: false }
            : msg
        )
      );
    }
    ...
    setIsTyping(false);
    setShouldStopGeneration(false);
    // Keep isGenerating as false so the button shows resume state
  };
  ...
  const simulateTypingAndGetResponse = async (text) => {
    ...
    setIsGenerating(true);
    setShouldStopGeneration(false);
    setPausedMessage(null); // Clear any previous paused message
  ...
  const handleSend = async () => {
    ...
    // Clear any paused message when sending a new message
  ...
  const createNewChat = () => {
    ...
    setError(null);
    setPausedMessage(null); // Clear paused message for new chat
  };
  ...
  const switchChat = (chatId) => {
    ...
    setError(null);
    setPausedMessage(null); // Clear paused message when switching chats
  };
  ...
  const deleteChat = (chatId) => {
    if (chatHistory.length > 1) {
      ...
      // Clear paused message if it belongs to deleted chat
      setPausedMessage(null);
    }
  };
  ...
  const handleAnimationComplete = (messageId) => {
    ...
    setIsTyping(false);
    setShouldStopGeneration(false);
    setPausedMessage(null); // Clear paused message when animation completes
  };
      ...
      <div className="flex-1 flex flex-col transition-all duration-300">
                  ...
                  {message.isAnimating ? (
                    <div className="text-sm leading-relaxed">
                      <TypewriterText 
                        ...
                        shouldResume={shouldResumeGeneration}
                        startFromIndex={pausedMessage && pausedMessage.messageId === message.id ? pausedMessage.pausedAtIndex : 0}
                      />
                    </div>
                  ) : (
        ...
        <div className="p-6 bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg border-t border-gray-200/50 dark:border-gray-700/50 shadow-lg">
          <div className="flex items-end space-x-3 max-w-4xl mx-auto">
            ...
            {/* Send/Stop/Resume Button */}
            <div className="flex space-x-2">
              {/* Stop/Resume Button - Shows when generating or when there's a paused message */}
              {(isGenerating || pausedMessage) && (
                <button
                  onClick={handleStopGeneration}
                  className={`p-3 rounded-2xl text-white shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105 ${
                    isGenerating 
                      ? 'bg-red-500 hover:bg-red-600' 
                      : 'bg-green-500 hover:bg-green-600'
                  }`}
                  title={isGenerating ? "Stop Generation" : "Resume Generation"}
                >
                  {isGenerating ? (
